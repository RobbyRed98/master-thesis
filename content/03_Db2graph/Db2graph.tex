...
\begin{itemize}
    ...
    Diese Optimierungstechnik nutzt die Prefixed-IDs, um die Abfragemenge zu reduzieren. Diese sogenannten Prefixed-IDs lassen sich im Mapping spezifizieren, siehe Zeile 5 - 6 in \autoref{src:mapping_example}. Des weiteren setzen sich die Prefixed-IDs aus einem Prefix und den \texttt{id\_cols} im Mapping zusammen. Wird hier \autoref{src:mapping_example} als Beispiel herangezogen, so wäre die Prefixed-ID \code{''LINKDB0.NODETABLE"::1} möglich.
\end{itemize}
