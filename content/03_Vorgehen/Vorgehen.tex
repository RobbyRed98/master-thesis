...
\begin{itemize}
    \item Linkbench als Benchmark zum Messen der Performance,
...
\end{itemize}
Allerdings ist es nun auch das Ziel der Performance-Analyse in dieser Arbeit die Aspekte beziehungsweise den Einfluss der Messparameter:
\begin{itemize}
    \item Ergebnismenge und
    \item Datensatzgröße zu beachten. 
\end{itemize}
Dafür werden bei der Performance-Analyse sowohl Messungen auf der Basis von real und konstant-verteilten Datensätzen durchgeführt. 

Eine weitere Aufgabe, die aufgrund des neuen Ziels Teil der Arbeit wird, stellt dabei die Suche nach bereits existierenden Linkbench-Adapter-Implementierungen für Db2 Graph und Neo4j dar. Sollte jedoch keine der für die Messungen benötigten Adapter-Implementierung verfügbar sein, wird deren Implementierung somit ebenfalls zum Teil der Arbeit. 

Abschließend gilt es darauf hinzuweisen, dass obwohl beim neuen Ziel einige Elemente von \cite{sigmod_tian} übernommen werden, die hier erzielten Ergebnisse nicht mit den aus \cite{sigmod_tian} vergleichbar sind. Schließlich sind die Bedingungen bei den Messungen nicht vergleichbar.
