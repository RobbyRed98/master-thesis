...
\section{Begriffe oder Messungen}
\todo{Beschreiben, was Messung, was Messreihe, was Ergebnis evtl. Metrik inkludieren.}
...
Die Operationen werden dabei getrennt voneinander gemessen, um die während der \textit{Request}-Phase erzeilte Ressourcenauslastung immer einer bestimmten Operationsart zuordnen zu können. 

\section{Operationen}
In diesem Abschnitt wird auf die verschiedenen Operationen eingegangen deren Performance im Rahmen der Analyse dieser Arbeit eine Rolle spielen. Außerdem wird auf die Begrenzung der Ergebnismenge bei der \texttt{getLinkList}-Operation eingegangen. Schließlich wird dieses Limit bei einigen Messungen variiert, wodurch es zu einem Messparameter wird, den es bei der Auswertung der Ergebnisse zu beachten gilt. 
...
\subsection{Begrenzung der Ergebnismenge}
Bei der Begrenzung der Ergebnismenge handelt es sich um eine Workload-Konfiguration, die lediglich die \texttt{getLinkList}-Operation betrifft. Durch sie wird eine obere Grenze für die Ergebnismenge in für die Query der \texttt{getLinkList}-Operation gesetzt. 

Sie wurde dabei im Rahmen dieser Arbeit in den Benchmark eingeführt, um dadurch die Performance der verschiedenen Datenbanksysteme bei größeren oder kleineren Ergebnismengen untersuchen zu können. 
...
100.000 wurde darüber hinaus hierbei als größte Menge gewählt, da eine Grenze von 1.000.000 den Messzeitraum der \texttt{Request}-Phase von 3 Stunden bei einigen Datenbanksystemen um ein Vielfaches überschritten hätte.
...
\section{Queries}
Im Rahmen dieses Abschnitts wird auf die Queries eingegangen die bei der Performance-Analyse eine wichtige Rolle spielen. Dabei wird kurz darauf eingegangen in welchen Abfragesprachen die Queries formuliert werden müssen. Anschließend wird dann erläutert, in welche zwei verschiedene Kategorien von Queries im Rahmen dieser Arbeit eine Rolle spielen. Darüber hinaus wird auch im Rahmen dieses Abschnitts auf auf den Aspekt der Begrenzung der Ergebnismenge für der \texttt{getLinkList}-Queries eingegangen, welcher bei den Messungen mit Bezug zu real-verteilten Datensätzen eine signifikante Rolle als Konfigurationsparameter für die gleichnamige Operation darstellt. 

Bevor im Rahmen des 



In diesem Abschnitt wird auf die verschiedenen Queries eingegangen, die den Kern der unterschiedlichen Operationen darstellen, die während einer \textit{Request}-Phase durchgeführt werden. Hierbei handelt es sich um die Anfragen, die eigentlich mit dem jeweiligen Datenbanksystem interagieren. Es handelt sich hierbei um einen für die Messungen relevanten Aspekt, der allerdings ein Stück weit dem Kapitel \nameref{implementierung} vorgreift. 

Da die Queries allerdings auch einen wichtigen Aspekt für die Einordnung der Messergebnisse darstellen und darüber hinaus die Art der Queries während den Messungen auch die Rolle eines Parameters einnimmt, werden sie bereits als Abschnitt dieses Kapitels aufgeführt. 

Die Queries können hierbei im Rahmen dieser Arbeit in zwei verschiedene Kategorien eingeteilt werden:
\begin{itemize}
    \item reguläre Queries und
    \item ID-Queries.
\end{itemize}
Die beiden Kategorien unterschieden sich hierbei hauptsächlich bezüglich der Art der Formulierung der Gremlin-Queries. Es gibt dabei allerdings auch kleinere Unterschiede zwischen der Logik der \texttt{getLink}-Queries, welche dazu führen, das sich auch die Cypher-Queries der beiden Kategorien unterscheiden. 


Beide dieser Query-Kategorien verfügen dabei über Cypher- und Gremlin-Queries für die Durchführung der vier gebenchmarkten Operationen:
\begin{itemize}
    \item \texttt{getNode},
    \item \texttt{getLink},
    \item \texttt{countLink} und
    \item \texttt{getLinkList} durch. 
\end{itemize}

\subsection{Reguläre Queries}
Die Queries, die Teil dieser Kategorie sind, werden hierbei während nahezu allen Messreihen herangezogen. 



\subsection{Konstant-verteilte Datensätze}
Für die 

\subsection{Real-verteilte Datensätze}
