...
\section{Messungen}
Im Rahmen dieses Abschnitts wird kurz beschrieben, die Messungen im Rahmen dieser Arbeit aufgebaut werden beziehungsweise gegliedert sind. Für die Messungen existieren im Rahmen dieser Arbeit die folgenden drei Begriffe beziehungsweise Einheiten:
\begin{itemize}
    \item Benchmark-Durchlauf,
    \item Serie und 
    \item Messreihe. 
\end{itemize}

\subsection{Benchmark-Durchlauf}
Ein Benchmark-Durchlauf stellt dabei die kleinste Einheit dar, auf die die Messungen in dieser Arbeit heruntergebrochen werden können. Ein Benchmark-Durchlauf deckt hierbei -- wie der Name bereits vermuten lässt -- den Durchlauf eines Benchmarks für eine bestimmte Graph-Operation ab. 

\subsection{Messserie}
Unter dem Begriff Messserie werden die Benchmark-Durchläufe für alle Graph-Operationen eines bestimmten Datenbanksystems zusammengefasst. Die Serie umfasst somit alle Benchmark-Durchläufe für die in \autoref{analyse:operationen} oder \autoref{analyse:parameter:ergebnissmenge} aufgezählten Operationen. 

\subsection{Messreihe}
In einer Messreihe werden die Messserien mehrerer Datenbanksysteme zusammengefasst. Dabei verfügen alle Datenbanksysteme deren Messserien Teil der Messreihe sind über selbe Konfiguration der in \autoref{analyse:parameter} beschriebenen Parameter. 
...
\todo{Evtl. Messungen überarbeiten}