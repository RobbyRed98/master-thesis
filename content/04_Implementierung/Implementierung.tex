...
Des Weiteren überschreibt er ebenfalls die lesenden Operationen, sodass sie mittels der in \autoref{src:gremlin_queries} und \autoref{src:gremlin_id_queries} formulierten Queries mit der Db2-Graph-V11.5.6.0-Instanz interagieren. Dafür baut der Adapter ähnlich wie der \texttt{Db2Graph\allowbreak Old}-Adapter eine Gremlin-Verbindung auf. Der Verbindungsaufbau erfordert hier allerdings aufgrund des in Db2 Graph V11.5.6.0 neu eingeführten Verbindungs- und Session-Managements weitaus mehr Logik als beim \texttt{Db2GraphOld}-Adapter. Eine Übersicht über die vom \texttt{Db2Graph}-Adapter aufgebauten kann dabei \autoref{fig:adapter_verbindungen} entnommen werden.
